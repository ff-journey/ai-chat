...
async def _map_worker(state: WorkerState, config: RunnableConfig):
    ...
    print(response)
    return {"messages": [response], "current_agent":"map_worker"}
...
async def get_weather_tour_advice(state: Annotated[MessagesState, InjectedState],config: RunnableConfig) :
    """根据城市名获取天气预报和路况信息"""
    ...
    if messages_out:
        ...
        return {"messages":[messages_out[-1]], "current_agent":"map_worker"}
    return {"messages":[]}